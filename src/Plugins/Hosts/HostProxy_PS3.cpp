...
int HostProxy_PS3::generate_descriptor(Device* device) {
	char *ptr;
	int i;
...
int HostProxy_PS3::connect(Device* device,int timeout) {
	int status;
...
void HostProxy_PS3::reset() {
	disconnect();
	reconnect();
}

bool HostProxy_PS3::is_connected() {
...
int HostProxy_PS3::control_request(usb_ctrlrequest *setup_packet, int *nbytes, __u8** dataptr,int timeout) {
...
void HostProxy_PS3::send_data(__u8 endpoint,__u8 attributes,__u16 maxPacketSize,__u8* dataptr,int length) {
	if (!endpoint) {
...
bool HostProxy_PS3::send_wait_complete(__u8 endpoint,int timeout) {
...
void HostProxy_PS3::receive_data(__u8 endpoint,__u8 attributes,__u16 maxPacketSize,__u8** dataptr, int* length, int timeout) {
	if (!endpoint) {
...
void HostProxy_PS3::control_ack() {
...
void HostProxy_PS3::stall_ep(__u8 endpoint) {
...
void HostProxy_PS3::setConfig(Configuration* fs_cfg,Configuration* hs_cfg,bool hs) {
