...
void HostProxy_Switch::aio_send_completion_handler(sigval_t sigval)
{
	struct aiocb* aio;
...
int HostProxy_Switch::generate_descriptor(Device* device) {
	char *ptr;
	int i;
...
int HostProxy_Switch::connect(Device* device,int timeout) {
	int status;
...
int HostProxy_Switch::reconnect() {
	int status;
...
void HostProxy_Switch::disconnect() {
...
void HostProxy_Switch::reset() {
	disconnect();
	reconnect();
}

bool HostProxy_Switch::is_connected() {
...
int HostProxy_Switch::control_request(usb_ctrlrequest *setup_packet, int *nbytes, __u8** dataptr,int timeout) {
...
void HostProxy_Switch::send_data(__u8 endpoint,__u8 attributes,__u16 maxPacketSize,__u8* dataptr,int length) {
	if (!endpoint) {
...
bool HostProxy_Switch::send_wait_complete(__u8 endpoint,int timeout) {
...
void HostProxy_Switch::receive_data(__u8 endpoint,__u8 attributes,__u16 maxPacketSize,__u8** dataptr, int* length, int timeout) {
	if (!endpoint) {
...
void HostProxy_Switch::control_ack() {
...
void HostProxy_Switch::stall_ep(__u8 endpoint) {
...
void HostProxy_Switch::setConfig(Configuration* fs_cfg,Configuration* hs_cfg,bool hs) {
...
void HostProxy_Switch::handle_USB_REQ_SET_CONFIGURATION()
{
	control_ack();
}
